  ...
  def minifeed_message(activity)
    ...
    when "Exchange"
      ...
      if exchange.group.nil?
        ...
        %(#{person_link(person)} earned #{exchange.amount} #{exchange.group.unit} from #{person_link(exchange.customer)}  for #{metadata_link(exchange.metadata.name,exchange.metadata)} in #{group_link(exchange.group)}.)
      end
    else
      raise "Invalid activity type #{activity_type(activity).inspect}"
    end
